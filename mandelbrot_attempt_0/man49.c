...
void rst() {
	...
	inst("imm out0 1000");
	doasnimm(GARBAGE0, 0xff00);
	marklow = addr;
	dosuccessor(GARBAGE0);
	_immaddr(GARBAGE0);
	_immgen(0x0000);
	_ramgen1;
	if (!(addr%2))
...
void main(int argc, char**argv) {
	...
	dosuccessor(MAIN_I); //putting this here instead of after checking zr
	...
	toimm(addr+2);
	toimm(nxtaddr);
	doputpixel(0xffff, 0xffff, 0xffff);
	toimm(doneaddr);
	addr = nxtaddr;
	if (!(addr%2))
		inst("dnc noop 0000");
	...
	inst("imm out0 ffff");
	inst("ram out1 0000");
	doasnimm(GARBAGE0, 0x0055);
	calladd(MAIN_I, MAIN_I, GARBAGE0);
	_immaddr(MAIN_I);
	inst("imm dir0 ffff");
	inst("ram dir1 0000");
	*/
