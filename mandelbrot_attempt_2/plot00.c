 ...
 * Integers are simply stored as 32 bits in order (sign is optional).
 * "Floating points" are stored as follows:
 * The pointer to the number points to an even address.
 * The even part just has the lowest 16 bits of the fraction.
 * Odd part (eg pointer|0x0001):
...
#define VALUE_XSTART1 0xf84f
#define VALUE_XSTART0 0x0000
#define VALUE_DX1 0xffff
#define VALUE_DX0 0x0000
#define VALUE_XLIMN1 0x08f1
#define VALUE_XLIMN0 0x0000
#define VALUE_YSTART1 0xf55d
#define VALUE_YSTAET0 0x0000
#define VALUE_DY1 0x0001
#define VALUE_DY0 0x0000
#define VALUE_YLIMN1 0x09b3
#define VALUE_YLIMN0 0x0000
...
void main(int argc, char**argv) {
	...
	instval("imm ramall", VALUE_XSTART);
	inst("imm addr0 ffff");
	instval("imm addr1", MAIN_Y);
	instval("imm ramall", VALUE_YSTART);
	inst("imm addr0 ffff");
	instval("imm addr1", MAIN_DX);
	instval("imm ramall", VALUE_DX);
	inst("imm addr0 ffff");
	instval("imm addr1", MAIN_DY);
	instval("imm ramall", VALUE_DY);
	inst("imm addr0 ffff");
	instval("imm addr1", MAIN_XLIMN);
	instval("imm ramall", VALUE_XLIMN);
	inst("imm addr0 ffff");
	instval("imm addr1", MAIN_YLIMN);
	instval("imm ramall", VALUE_YLIMN);
	//print the rendering for the pixel you are at
	loopaddr = addr;
	//get rendering and load it into gen
	render(MAIN_RESULT, MAIN_Y, MAIN_X);
	//shift the rendering by *1 (eg dont shift)
	//1 becomes almost black, 255 is light blue, 256 rolls over to black
	...
	inst("ram gen1 0000");
	buswritegen(); //blue
	//shift the rendering by *4 total
	//if >= 32, make medium green
	//if = 256, make it black
	//use addr as register helper
	//don't need to account for last bit wrap around
	inst("imm addr0 ffff");
	instval("imm addr1", MAIN_RESULT);
	...
	inst("dnc noop 0000");
	buswritegen(); //green
	//shift the rendering by *2 total
	//if >= 64, make medium red
	//if = 256, make it black
	//use addr as register helper
	//don't need to account for last bit wrap around
	inst("imm addr0 ffff");
