 ...
 * Note: escape is determined using a square, not a circle.
 */
...
#define MAIN_COUNTER 0x4206
#define MAIN_DI 0x4207
#define MAIN_DR 0x4208
#define MAIN_RISUM 0x4209
#define MAIN_RIDIFF 0x420a
...
void rendermain() {
	//just a macro for MAIN* variables in specific
	...
	inst("gen ramall 0000");
	//start counter at one so immediate escape has value of 1, not zero
	//(which is reserved for the members of the set).
	inst("imm addr0 ffff");
	instval("imm addr1", MAIN_COUNTER);
	inst("imm ramall 0001");
	startaddr = addr;
	//test if counter is >= max (8)
	inst("imm addr0 ffff");
	instval("imm addr1", MAIN_COUNTER);
	makeaddrodd();
	inst("ram jzor fff8");
	...
	//test if escape due to real or imaginary being outside of (-2,2) exclusive
	//test if zi >= 2
	inst("imm addr0 ffff");
	instval("imm addr1", MAIN_ZI);
	makeaddrodd();
	inst("ram jzor 8000");
	instnxt("ram jzor 6000", addr+2);
	instnxt("dnc noop 0000", addr+3);
	instnxt("dnc noop 0000", addr+2);
	instexpnxt("dnc noop 0000", "PRNT");
	//test if zi <= -2 (do two's complement then test if >= 2)
	inst("imm gen1 ffff");
	inst("ram gen0 0000");
	genpred16();
	inst("imm addr0 ffff");
	...
	makeaddrodd();
	inst("gen jzor 8000");
	instnxt("gen jzor 6000", addr+2);
	instnxt("dnc noop 0000", addr+3);
	instnxt("dnc noop 0000", addr+2);
	instexpnxt("dnc noop 0000", "PRNT");
	//test if zi ends in four ones and escape if so (this is just to inject artifacts)
	inst("imm addr0 ffff");
	instval("imm addr1", MAIN_ZI);
	inst("imm gen1 ffff");
	inst("ram gen0 0000");
	...
	instnxt("dnc noop 0000", addr+2);
	instexpnxt("dnc noop 0000", "PRNT");
	//test if zr <= -2 (do two's complement then test if >= 2)
	inst("imm gen1 ffff");
	inst("ram gen0 0000");
	genpred16();
	inst("imm addr0 ffff");
	...
	instnxt("dnc noop 0000", addr+2);
	instexpnxt("dnc noop 0000", "PRNT");
	//test if zr ends in four ones and escape if so (this is just to inject artifacts)
	inst("imm addr0 ffff");
	instval("imm addr1", MAIN_ZR);
	inst("imm gen1 ffff");
	inst("ram gen0 0000");
	...
	inst("gen jzor 000f");
	instexpnxt("dnc noop 0000", "PRNT");
	//calculate iterated real components (eg (zr-zi) and (zr+zi))
	//(don't multiply yet because need MAIN_ZR intact for MAIN_ZI calculation)
	//do the zr+zi
	calladd(MAIN_RISUM, MAIN_ZI, MAIN_ZR);
	//get the twos complement of zi in ridiff
	inst("imm addr0 ffff");
	instval("imm addr1", MAIN_ZI);
	inst("imm gen0 ffff");
	inst("ram gen1 0000");
	...
	inst("addr gen0 0000");
	inst("imm addr0 ffff");
	instval("imm addr1", MAIN_RIDIFF);
	inst("gen ramall 0000");
	//do the zr-zi
	calladd(MAIN_RIDIFF, MAIN_RIDIFF, MAIN_ZR);
	...
	callmul(MAIN_ZR, MAIN_RISUM, MAIN_RIDIFF);
	...
	inst("imm gen1 ffff");
	inst("ram gen0 ffff");
	instnxt("gen ramall 0000", startaddr);
	//replace end with actual value
	sprintf(str, "%04x", addr);
	replacex88("PRNT", str);
}
...
void main(int argc, char**argv) {
	...
	inst("imm addr0 ffff");
	instval("imm addr1", MAIN_DR);
	inst("imm ramall ffd0");
	inst("imm addr0 ffff");
	instval("imm addr1", MAIN_DI);
	...
	inst("imm gen0 ffff");
	inst("ram gen1 0000");
	//shift the rendering value up 5 spaces; becomes redder
	//with increasing value until 8, which rolls over to zero
	...
	buswritegen();
	//increment column
	calladd(MAIN_ZI0, MAIN_ZI0, MAIN_DI);
	//test column value
	inst("imm addr0 ffff");
	instval("imm addr1", MAIN_ZI0);
	inst("imm gen1 ffff");
	...
	makeaddrodd();
	inst("ram jzor e97f");
	instnxt("gen jzor 1680", addr+2);
	instnxt("dnc noop 0000", loopaddr);
	instnxt("dnc noop 0000", addr+2);
	instnxt("dnc noop 0000", loopaddr);
	//increment row and reset column
	calladd(MAIN_ZR0, MAIN_ZR0, MAIN_DR);
	inst("imm addr0 ffff");
	instval("imm addr1", MAIN_ZI0);
	inst("imm ramall e980");
	//test row value
