...
#define MAIN_DX 0x000a
#define MAIN_DY 0x000b
#define MAIN_XEND 0x000c //highest corner position
#define MAIN_YEND 0x000d //highest corner position
#define MAIN_DUMMY 0x000e
...
void main(int argc, char** argv) {
	...
	inst("imm dir0 ffff");
	lcdinit();
	//make screen and speed zero
	//random values for starting position cause why not
	//not gonna worry about overflow, just see what happens
	lcdboximmimm(0, 0, 0,  0, 239, 0, 319);
	inst("imm addr0 ffff");
	isntval("imm addr1", MAIN_DX);
	inst("imm ramall 0000");
	inst("imm addr0 ffff");
	instval("imm addr1", MAIN_DY);
	inst("imm ramall 0000");
	//Loop that moves the "lander" sprite (just a white box)
	loopaddr = addr;
	inst("imm addr0 ffff");
	...
	inst("ram gen1 0000");
	inst("imm addr0 ffff");
	instval("imm addr1", MAIN_XEND);
	inst("imm ramall 000a");
	addgenram();
	inst("gen ramall 0000");
	inst("imm addr0 ffff");
	...
	inst("ram gen1 0000");
	inst("imm addr0 ffff");
	instval("imm addr1", MAIN_YEND);
	inst("imm ramall 000a");
	addgenram();
	inst("gen ramall 0000");
	lcdboximmram100(0, 0, 0,  MAIN_Y, MAIN_YEND, MAIN_X, MAIN_XEND);
	keygen();
	makeaddrodd();
	inst("gen jzor 4000");
	instepxnxt("dnc noop 0000", "MA_G");
	inst("dnc noop 0000");
	inst("imm addr0 ffff");
	...
	addgenram();
	inst("gen ramall 0000");
	lcdboximmram(0, 0, 0,  MAIN_Y, MAIN_YEND, MAIN_X, MAIN_XEND);
