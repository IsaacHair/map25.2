...
#define MAIN_BOOST 0x0024 //value to scale acceleration by
...
void comm2_2datpointdwn9(int comm, unsigned short spoint, unsigned short epoint) {
	//writes command, then 0x00, then ram[spoint]>>9, then 0x00, then ram[epoint]>>9
	instval("imm out0", _LCD_RS);
	buswrite(comm);
	instval("imm out1", _LCD_RS);
	buswrite(0x00);
...
void lcdsizeframepoint_dwn9(unsigned short scpoint, unsigned short ecpoint,
		       unsigned short sppoint, unsigned short eppoint) {
	//size frame with pointers
	comm2_2datpointdwn9(0x2a, scpoint, ecpoint);
	comm2_2datpointdwn9(0x2b, sppoint, eppoint);
}
...
void lcdboxgreypointimm_dwn9(int shade,
		       unsigned short scpoint, unsigned short ecpoint,
		       unsigned short sppoint, unsigned short eppoint,
		       int size) {
	//Start-end range is INCLUSIVE.
	...
	unsigned short loopaddr;
	lcdpauseframe();
	lcdsizeframepoint_dwn9(scpoint, ecpoint, sppoint, eppoint);
	lcdresetframe();
	//double up on writes for speed and to fit in 16 bit counter
	//ok if write 3 extra pixels b/c just box
	//use addr register as helper
...
void main(int argc, char** argv) {
	...
	//set MAIN_DDX
	//start at gravity
	setimmimm(MAIN_DDX, 0xfffe);
	//accelerate up
	keygen();
	makeaddrodd();
	inst("gen jzor 0200");
	instexpnxt("dnc noop 0000", _next);
	setimmimm(MAIN_DUMMY, 0x0009);
	...
	//render
	setimmimm(MAIN_XEND, (5-1)*(1<<9));
	setimmimm(MAIN_YEND, (5-1)*(1<<9));
	calladd(MAIN_XEND, MAIN_XEND, MAIN_X);
	calladd(MAIN_YEND, MAIN_YEND, MAIN_Y);
	lcdboxgreypointimm_dwn9(0,  MAIN_XOLD, MAIN_XENDOLD, MAIN_YOLD, MAIN_YENDOLD, 25);
	lcdboxgreypointimm_dwn9(63,  MAIN_X, MAIN_XEND, MAIN_Y, MAIN_YEND, 25);
	inst("dnc noop 0000");
	//loop infinitely
