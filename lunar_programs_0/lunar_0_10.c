...
void main(int argc, char** argv) {
	...
	//init values
	setimmimm(MAIN_X, 0x0420);
	setimmimm(MAIN_Y, 0x0690);
	...
	inst("gen jzor 0020");
	instexpnxt("dnc noop 0000", _next);
	setimmimm(MAIN_DUMMY, 0x0003);
	calladd(MAIN_DDY, MAIN_DDY, MAIN_DUMMY);
	replacex88expimm(_next, addr);
	//acceleration boost
	setimmimm(MAIN_BOOST, 0x1000); //this is one in fixed point (1,3,12) instead of (1,9,6)
	makeaddrodd();
	inst("gen jzor 0010");
	instexpnxt("dnc noop 0000", _next);
	setimmimm(MAIN_BOOST, 0x3744); //also in fixed point (1,3,12)
	replacex88expimm(_next, addr);
	//do the calculations
