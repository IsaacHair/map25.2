...
void main(int argc, char**argv) {
	...
	inst("imm gen0 ffff");
	inst("ram gen1 0000");
	//shift the rendering by *16 (1 becomes almost black, 16-31 is light blue)
	...
	inst("imm gen0 ffff");
	inst("addr gen1 0000");
	//make it full blue if MAIN_RESULT is more than what fits in pixel<<4
	inst("imm addr0 ffff");
	instval("imm addr1", MAIN_RESULT);
	makeaddrodd();
	inst("ram jzor fff0");
	instnxt("dnc noop 0000", addr+2);
	instnxt("imm gen1 ffff", addr+1);
